...
def generate_rsi_strategy(df, rsi_buy=30, rsi_sell=70, stop_loss=-12.0, mode='recovery'):
    """
    生成 RSI 策略信号
    mode='recovery': RSI 从超卖回升突破 rsi_buy 时买入（避免接飞刀）
    mode='oversold':  RSI 跌破 rsi_buy 时买入（均值回归，原策略）
    """
    ...
    print(f"\n📊 RSI 策略 (模式: {mode})")
    if mode == 'recovery':
        print(f"   买入条件: RSI 从超卖回升突破 {rsi_buy}")
    else:
        print(f"   买入条件: RSI < {rsi_buy}（均值回归）")
    ...
    for i in range(1, n):
        if mode == 'recovery':
            # RSI 从超卖区回升突破阈值（确认复苏）
            if rsi[i-1] <= rsi_buy and rsi[i] > rsi_buy:
                signals[i] = 1
        else:
            # 原策略：RSI 跌破阈值（均值回归）
            if rsi[i] < rsi_buy and rsi[i-1] >= rsi_buy:
                signals[i] = 1
...
def backtest_rsi(df, start="2025-01-01", end="2025-12-31", stop_loss=-12.0, year_label="2025"):
    """回测 RSI 策略"""
    ...
    print(f"\n{'='*60}")
    print(f"📊 {year_label} 年 RSI 均值回归策略回测")
    print(f"{'='*60}")
    ...
    plt.title(f'{year_label} 年 RSI 均值回归策略 - 资金曲线', fontsize=14)
    plt.legend()
    plt.grid(True, alpha=0.3)

    for t in trades:
        if t['type'] == 'BUY':
            plt.scatter(t['date'], equity[dates.index(t['date'])], 
    ...
    plt.tight_layout()
    plot_path = os.path.join(OUTPUT_DIR, f"backtest_{year_label}_rsi.png")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="RSI 均值回归策略回测")
    parser.add_argument("--year", type=str, default="2025", help="回测年份（默认 2025）")
    parser.add_argument("--mode", type=str, default="oversold", choices=["oversold", "recovery"],
                        help="策略模式: oversold=均值回归(跌破买入), recovery=复苏(回升突破买入)")
    parser.add_argument("--rsi-buy", type=float, default=20, help="RSI 买入阈值（默认 20）")
    parser.add_argument("--rsi-sell", type=float, default=70, help="RSI 卖出阈值（默认 70）")
    parser.add_argument("--stop-loss", type=float, default=-12.0, help="止损百分比（默认 -12%）")
    ...
    # 只取指定年份的数据生成信号
    df_year = df[df['date'].dt.year == int(args.year)].copy()
    if len(df_year) == 0:
        print(f"❌ {args.year} 年无数据")
        sys.exit(1)

    df = generate_rsi_strategy(df, rsi_buy=args.rsi_buy, rsi_sell=args.rsi_sell,
                                stop_loss=args.stop_loss, mode=args.mode)

    start_date = f"{args.year}-01-01"
    end_date = f"{args.year}-12-31"
    backtest_rsi(df, start=start_date, end=end_date, stop_loss=args.stop_loss, year_label=args.year)
